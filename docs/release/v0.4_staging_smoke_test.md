...
## Parser worker deployment and recovery (#368)

Run the [bounded worker verification](../deployment/parser_worker.md#bounded-staging-validation-record-actual-results)
on the designated staging/Pi host. Record the release SHA and PASS/FAIL evidence
for startup, single-worker inventory across hosts, environment isolation, queued
work across API/worker restart, controlled process failure with automatic restart,
interrupted-attempt recovery, independent service restarts, reboot and log privacy.
Do not mark these checks passed from unit tests or service configuration alone.
